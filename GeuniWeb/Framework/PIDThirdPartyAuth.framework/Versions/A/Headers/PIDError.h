...
typedef NS_ENUM(NSInteger, PIDErrorType)
{
    ...
    PIDErrorNotLoggedin = -8,                       /* 로그인 되어 있지 않으나 로그아웃을 호출 했을 경우 발생 */
    PIDErrorInvalidState = -9,                      /* 로그인 시 state값이 일치하지 않을 경우 */
    PIDErrorInvalidSignature = -10                  /* 로그인 시 signature값이 잚못생성되었을 경우 */
